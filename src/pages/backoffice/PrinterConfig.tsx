              ...
              <div key={printer.label} className="bg-gray-800 rounded-xl p-4 space-y-3">
                ...
                <button
                  onClick={() => {
                    ...
                    const serverJs = `// Celebiz Print Server — Auto-generated
...
`
                    ...
                    const batContent = [
                      ...
                      'net stop CelebizPrint 2>nul',
                      'sc delete CelebizPrint 2>nul',
                      'schtasks /delete /tn "CelebizPrint" /f 2>nul',
                      'taskkill /f /im node.exe 2>nul',
                      'timeout /t 2 /nobreak >nul',
                      '',
                      'REM Create directory',
                      'if not exist "C:\\CelebizPrint" mkdir "C:\\CelebizPrint"',
                      '',
                      'REM Write config.json',
                      '(echo {) > "C:\\CelebizPrint\\config.json"',
                      '(echo   "printer_ip": "' + ip + '",) >> "C:\\CelebizPrint\\config.json"',
                      '(echo   "printer_port": 9100,) >> "C:\\CelebizPrint\\config.json"',
                      '(echo   "server_port": 6543) >> "C:\\CelebizPrint\\config.json"',
                      '(echo }) >> "C:\\CelebizPrint\\config.json"',
                      '',
                      ...
                      "'@; $s | Out-File -FilePath 'C:\\CelebizPrint\\server.js' -Encoding UTF8\"",
                      '',
                      'REM Write watchdog script — restarts node if it crashes, runs forever hidden',
                      'echo Writing watchdog...',
                      '(echo Do) > "C:\\CelebizPrint\\watchdog.vbs"',
                      '(echo   Set objShell = CreateObject^("WScript.Shell"^)) >> "C:\\CelebizPrint\\watchdog.vbs"',
                      '(echo   objShell.Run "cmd /c cd /d C:\\CelebizPrint ^&^& node server.js", 0, True) >> "C:\\CelebizPrint\\watchdog.vbs"',
                      '(echo   WScript.Sleep 3000) >> "C:\\CelebizPrint\\watchdog.vbs"',
                      '(echo Loop) >> "C:\\CelebizPrint\\watchdog.vbs"',
                      '',
                      'REM Create scheduled task — runs at SYSTEM STARTUP (before login), as SYSTEM user',
                      'echo Setting up auto-start...',
                      'schtasks /create /tn "CelebizPrint" /tr "wscript.exe C:\\CelebizPrint\\watchdog.vbs" /sc onstart /ru SYSTEM /rl highest /f',
                      '',
                      "REM Also create a logon backup task (in case onstart doesn't fire on some machines)",
                      'schtasks /create /tn "CelebizPrintLogon" /tr "wscript.exe C:\\CelebizPrint\\watchdog.vbs" /sc onlogon /rl highest /f',
                      '',
                      'REM Also put in Startup folder as triple backup',
                      'copy /y "C:\\CelebizPrint\\watchdog.vbs" "%APPDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\CelebizPrint.vbs" >nul 2>nul',
                      '',
                      'REM Start it now',
                      'echo Starting print server...',
                      'taskkill /f /im wscript.exe 2>nul',
                      'timeout /t 1 /nobreak >nul',
                      'start "" wscript.exe "C:\\CelebizPrint\\watchdog.vbs"',
                      '',
                      'REM Wait and verify',
